...
def find_used_cards(card_indices, seen_card_indices=None, old_int_to_new_int=None):
    ...
    for row_idx in trange(len(card_indices), unit='row', unit_scale=True, smoothing=1e-04, dynamic_ncols=True):
...
def dedupe_directory(directory):
    ...
    for subtype in ('training', 'validation'):
        ...
        for filename, count in zip(('pairs', 'cards_in_pack', 'picked', 'seen'), (pair_count, context_count,
            ...
            arr, seen_card_indices, old_int_to_new_int = find_used_cards(
                load_npy_to_tensor(subdirectory/f'{filename}.npy.zstd')[:count], seen_card_indices,
                old_int_to_new_int)
            with open(subdirectory/f'{filename}.npy.zstd.2', 'wb') as fh:
                with cctx.stream_writer(fh) as compressor:
                    np.save(compressor, arr.astype(np.uint16), allow_pickle=False)
    int_to_card = [None for _ in range(len(old_int_to_new_int))]
