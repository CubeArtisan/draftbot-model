...
class ContextualRating(tf.keras.layers.Layer):
    def __init__(self, num_items, embed_dims, context_dims, time_shape, hyperbolic=False,
    ...
    def build(self, input_shape):
        if self.hyperbolic:
            self.item_curvature = self.add_weight('item_curvature', shape=(),
                                                  initializer=tf.constant_initializer(-2), trainable=True)
            self.item_embeddings = self.add_weight('item_embeddings', shape=(self.num_items - 1, self.embed_dims - 1),
                                                   ...
                                                   trainable=True)
            self.pool_embedding = SetHyperEmbedding(self.num_items, self.context_dims, self.time_shape,
