...
class ContextualRating(tf.keras.layers.Layer):
    def __init__(self, num_items, embed_dims, context_dims, hyperbolic=False, item_dropout_rate=0.0,
                 dense_dropout_rate=0.0, activation='selu', bounded_distance=True,
                 final_activation='linear', normalize_sum=True, hidden_units=None, **kwargs):
    ...
    def build(self, input_shape):
        if self.hyperbolic:
            self.item_curvature = self.add_weight('item_curvature', shape=(),
                                                  initializer=tf.constant_initializer(0.54), trainable=True)
            self.item_embeddings = self.add_weight('item_embeddings', shape=(self.num_items - 1, self.embed_dims - 1),
                                                   initializer=tf.random_normal_initializer(0, 1 / self.embed_dims / self.embed_dims),
                                                   trainable=True)
            ...
            self.zero_embed = tf.constant(0, shape=(1, self.embed_dims - 1), dtype=self.compute_dtype)
        else:
    ...
    def call(self, inputs, training=False):
        ...
        if self.hyperbolic:
            item_curvature = 8 * tf.math.softplus(self.item_curvature, name='item_curvature')
            item_embeds = tf.gather(embeddings, item_indices, name='item_embeddings')
            item_embeds = to_hyper(item_embeds, curvature=item_curvature, name='hyperbolic_item_embeddings')
            ...
            if training and self.hyperbolic:
                tf.summary.scalar('weights/curvature', tf.constant(-1, dtype=self.compute_dtype) / item_curvature)
        else:
            item_embeds = tf.gather(embeddings, item_indices, name='item_embeddings')
...
class TimeVaryingLinear(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        sub_layer_weights_orig = 8 * tf.math.softplus(self.sub_layer_weights, name='sub_layer_weights')
...
class DraftBot(tf.keras.models.Model):
    def __init__(self, num_items, embed_dims=64, seen_dims=16, picked_dims=32,
                 triplet_loss_weight=0.0, dropout_picked=0.0, dropout_seen=0.0,
                 hyperbolic=False, margin=1, log_loss_weight=1.0, activation='selu',
                 ...
                 picked_hidden_units=64, seen_hidden_units=32, picked_dropout_dense=0.0,
                 seen_dropout_dense=0.0, **kwargs):
        ...
        self.final_activation = final_activation
        self.normalize_sum = normalize_sum
        self.picked_hidden_units = picked_hidden_units
        self.seen_hidden_units = seen_hidden_units
        self.picked_dropout_dense = picked_dropout_dense
    ...
    def get_config(self):
        ...
        config.update({
            ...
            "final_activation": self.final_activation,
            "normalize_sum": self.normalize_sum,
            "picked_hidden_units": self.picked_hidden_units,
            "seen_hidden_units": self.seen_hidden_units,
            "picked_dropout_dense": self.picked_dropout_dense,
    ...
    def build(self, input_shapes):
        ...
        if self.pool_context_ratings:
            sublayers.append(ContextualRating(self.num_items, self.embed_dims, self.picked_dims,
                                              ...
                                              dense_dropout_rate=self.picked_dropout_dense,
                                              activation=self.activation,
                                              bounded_distance=self.bounded_distance,
                                              final_activation=self.final_activation,
                                              normalize_sum=self.normalize_sum,
                                              hidden_units=self.picked_hidden_units,
        ...
        if self.item_ratings:
            sublayers.append(ItemRating(self.num_items, name='CardRating'))
    ...
    def call(self, inputs, training=False):
        ...
        y_idx = tf.cast(inputs[5], dtype=tf.int32, name='y_idx')
